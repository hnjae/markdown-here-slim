...
function onLoad() {
  ...
  OptionsStore.get((options) => {
    ...
    const changesTitle = Utils.getMessage(
      "upgrade_notification_changes_tooltip",
    );
    const dismissTitle = Utils.getMessage(
      "upgrade_notification_dismiss_tooltip",
    );

    // Update text content
    document.getElementById("upgrade-notification-text").textContent =
      upgradeText;
    ...
    document
      .getElementById("upgrade-notification-link")
    ...
    document
      .getElementById("upgrade-notification-close")
      .addEventListener("click", (e) => {
        e.preventDefault();
        clearUpgradeNotification();
      });
