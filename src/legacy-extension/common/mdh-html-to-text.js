...
*/
  ...
  */
    ...
    if (pMatches > brMatches) {
      this.preprocessInfo.html = this.preprocessInfo.html
        .replace(/<p\b[^>]*>/gi, "<div>")
        .replace(/<\/p\b[^>]*>/gi, "</div>");
    }
    ...
    if (this.checkingIfMarkdown) {
      ...
      this.preprocessInfo.html = convertHTMLtoMarkdown(
        "a",
    ...
    this.preprocessInfo.html = this.preprocessInfo.html
      ...
      .replace(/(<\/div>)<div\b[^>]*><\/div>(<div[^>]*>)/gi, "$1$2")

      // <div><br></div> --> <br>
      .replace(/<div\b[^>]*><br\b[^>]*><\/div>/gi, "<br>")
  ...
  MdhHtmlToText.prototype.get = function () {
    return htmlToText(this.preprocessInfo.html, {
  ...
  MdhHtmlToText.prototype.postprocess = function (renderedMarkdown) {
    ...
    for (i = 0; i < this.preprocessInfo.exclusions.length; i++) {
      renderedMarkdown = renderedMarkdown.replace(
        this.preprocessInfo.exclusions[i].placeholder,
          ...
          if (depth === 1) {
            ...
            this.preprocessInfo.exclusions.push({
              ...
              content:
                ...
                (wrapInPara ? "<p>" : "") +
                this.preprocessInfo.html.slice(
                  currentOpenIndex,
