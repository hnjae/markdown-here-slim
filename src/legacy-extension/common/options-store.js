...
(function () {
  ...
  var ChromeOptionsStore = {
    ...
    get: function (callback) {
      this._storageGet((sync) => {
        ...
        this._fillDefaults(finalobj, callback);
      });
    },
    ...
    defaults: {
      "main-css": {
        __defaultFromFile__: "/common/default.css",
        __dataType__: "text",
      },
      "syntax-css": {
        __defaultFromFile__: "/common/highlightjs/styles/github.css",
        __dataType__: "text",
      },
      "math-enabled": DEFAULTS["math-enabled"],
      "math-value": DEFAULTS["math-value"],
      "forgot-to-render-check-enabled-2":
        DEFAULTS["forgot-to-render-check-enabled-2"],
    ...
    _storageGet: (callback) => {
      if (chrome.storage) {
        (chrome.storage.sync || chrome.storage.local).get(null, (obj) => {
          var key;
          for (key in obj) {
    ...
    _storageRemove: (keysToDelete, callback) => {
      if (chrome.storage) {
        (chrome.storage.sync || chrome.storage.local).remove(
          keysToDelete,
          callback,
        );
        return;
      ...
      this._storageGet((sync) => {
        ...
        for (var objKey in obj) {
          for (var syncKey in sync) {
            if (
              syncKey === objKey ||
              syncKey.indexOf(objKey + this._div) === 0
            ) {
        ...
        if (keysToDelete.length > 0) {
          this._storageRemove(keysToDelete, callback);
        } else {
          if (callback) callback();
        }
      });
  ...
  this.OptionsStore._fillDefaults = function (prefsObj, callback) {
    ...
    if (
      typeof prefsObj["math-value"] === "string" &&
      prefsObj["math-value"].indexOf("chart.googleapis.com") >= 0
    ) {
    ...
    for (key in that.defaults) {
      if (Object.hasOwn(that.defaults, key)) {
        allKeys.push(key);
      }
    }
