...
(function () {
  ...
  function replaceRange(range, html) {
    ...
    const documentFragment = Utils.safelyParseHTML(
      html,
  ...
  function makeStylesExplicit(wrapperElem, css) {
    ...
    for (i = 0; i < stylesheet.cssRules.length; i++) {
      ...
      selectorMatches = wrapperElem.parentNode.querySelectorAll(
        rule.selectorText,
      );
  ...
  */
  ...
  function renderMarkdown(
    ...
    var originalHtml = Utils.getDocumentFragmentHTML(
      selectedRange.cloneContents(),
    );

    // Call to the extension main code to actually do the md->html conversion.
    markdownRenderer(focusedElem, selectedRange, (mdHtml, mdCss) => {
      ...
      rawHolder =
        "<div " +
        'title="' +
        WRAPPER_TITLE_PREFIX +
        Utils.utf8StringToBase64(originalHtml) +
        '" ' +
      ...
      wrapper.ownerDocument.defaultView.setTimeout(
  ...
  function markdownHere(document, markdownRenderer, logger, renderComplete) {
    ...
    if (wrappers && wrappers.length > 0) {
      ...
      for (i = 0; i < wrappers.length; i++) {
        ...
        if (
          wrappers[i].getAttribute("markdown-here-wrapper-content-modified") &&
          !yesToAll
        ) {
          if (
            wrappers[i].ownerDocument.defaultView.confirm(
