      ...
      (html) => {
        html = html
          .replace(
            ...
            Utils.getMessage("forgot_to_render_prompt_title"),
          )
          .replace(
            "{{forgot_to_render_prompt_info}}",
            Utils.getMessage("forgot_to_render_prompt_info"),
          )
          .replace(
          ...
          .replace(
            ...
            Utils.getMessage("forgot_to_render_prompt_close_hover"),
          )
          .replace(
...
*/
  ...
  function forgotToRenderIntervalCheck(
    focusedElem,
    MarkdownHere,
    MdhHtmlToText,
    marked,
  ...
  */
    ...
    var forgotToRenderButtonSelector =
      getForgotToRenderButtonSelector(focusedElem);
    ...
    if (typeof focusedElem[WATCHED_PROPERTY] === "undefined") {
      ...
      setupForgotToRenderInterceptors(
        focusedElem,
        MdhHtmlToText,
        marked,
        prefs,
  ...
  function setupForgotToRenderInterceptors(
    composeElem,
    MdhHtmlToText,
    marked,
    prefs,
    ...
    var shouldIntercept = () => {
      var mdMaybe = new MdhHtmlToText.MdhHtmlToText(
        composeElem,
        null,
        true,
      ).get();
    ...
    var isSendButtonOrChild = (eventTarget) => {
    ...
    var composeSendButtonKeyListener = (event) => {
      if (
        isSendButtonOrChild(event.target) &&
    ...
    var composeSendButtonClickListener = (event) => {
      if (
        isSendButtonOrChild(event.target) &&
    ...
    var sendHotkeyKeydownListener = (event) => {
      ...
      var ourTarget =
        ...
        (event.target instanceof
          composeElem.ownerDocument.defaultView.HTMLHtmlElement &&
      ...
      if (
        ...
        debugLog(
          "setupForgotToRenderInterceptors",
          "sendHotkeyKeydownListener",
          "capture desired event",
        );
        showForgotToRenderPromptAndRespond(composeElem, composeSendButton);
      }
      debugLog(
        "setupForgotToRenderInterceptors",
        "sendHotkeyKeydownListener",
        "skipping undesired event",
        event.target,
      );
    };

    composeSendButton.parentNode.addEventListener(
      "keydown",
      composeSendButtonKeyListener,
      true,
    );
    composeSendButton.parentNode.addEventListener(
      "keyup",
    ...
    composeSendButton.parentNode.addEventListener(
      "click",
      composeSendButtonClickListener,
      true,
    );
    composeElem.parentNode.addEventListener(
      "keydown",
      sendHotkeyKeydownListener,
      true,
    );
  ...
  */
    ...
    var bulletList = mdMaybe.match(/^[*+-] /gm);
  ...
  function showForgotToRenderPromptAndRespond(composeElem, composeSendButton) {
    var sendOrGoBackToCompose = (send) => {
      if (send) {
  ...
  function showHTMLForgotToRenderPrompt(
    ...
    Utils.setFocus(
      composeSendButton.ownerDocument.querySelector(
        "#markdown-here-forgot-to-render-buttons button",
      ),
    );
    ...
    composeSendButton.ownerDocument.defaultView.setTimeout(
      okayToKeyupTimeout,
      300,
    );
    ...
    keyboardCapture = (() => {
      var keyboardCaptureHandler = (event) => {
        eatEvent(event);
        ...
        if (event.keyCode === ESCAPE_KEYCODE) {
          ...
          if (
            event.target.ownerDocument.activeElement === backButton ||
      ...
      return {
        add: () => {
          ...
          composeSendButton.ownerDocument.body.addEventListener(
            "keydown",
            eatEvent,
            true,
          );
        ...
        remove: () => {
          composeSendButton.ownerDocument.body.removeEventListener(
            "keydown",
            eatEvent,
            true,
          );
    ...
    dismissPrompt = (doc, send) => {
      keyboardCapture.remove();

      var forgotToRenderContent = doc.querySelector(
        "#markdown-here-forgot-to-render",
      );
    ...
    closeLink = composeSendButton.ownerDocument.querySelector(
    ...
    backButton = composeSendButton.ownerDocument.querySelector(
      "#markdown-here-forgot-to-render-button-back",
    );
    ...
    sendButton = composeSendButton.ownerDocument.querySelector(
      "#markdown-here-forgot-to-render-button-send",
    );
